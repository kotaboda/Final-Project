...
public class GameEngine {
	...
	public static String checkNote() {
		...
		Tile t = GameEngine.getGame().getFloors()[p.getFloorNum()-1].getTiles()[p.getCoordinates().getX()][p.getCoordinates().getY()];
		if (t instanceof FloorMessage || t instanceof WallMessage) {
			// retreive the notes at those coordinates in order to display it
			String message = GameEngine.getGame().getFloors()[p.getFloorNum()-1].getNotes().get(p.getCoordinates()).getMessage();
			return message;
		} else {
			return null;
		}
